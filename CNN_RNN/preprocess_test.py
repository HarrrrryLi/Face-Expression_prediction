...
all_fft = []
file_count=1
for filename in os.listdir('./test/video'):
    print("Preprocess Video "+str(file_count))
    file_count+=1
    video_landmarks = []
    if filename != ".DS_Store":
        vidcap = cv2.VideoCapture('./test/video/'+filename)
...
np.save('./test/test_landmarks.npy',np.array(all_landmarks,dtype=np.float32))
np.save('./test/test_fft.npy',np.asarray(all_fft,dtype=np.float32))