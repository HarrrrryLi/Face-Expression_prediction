...
def cnn_model_fn(features, labels, mode):
    ...
    conv1 = tf.layers.conv2d(inputs=input_layer,filters=64,kernel_size=[3, 1],padding="same",activation=tf.nn.relu,strides=[2,1])
    ...
    conv3 = tf.layers.conv2d(inputs=pool2,filters=16,kernel_size=[2, 1],padding="same",activation=tf.nn.relu,strides=[2,1])
    conv4 = tf.layers.conv2d(inputs=conv3,filters=8,kernel_size=[3, 1],padding="same",activation=tf.nn.relu,strides=[2,1])
    conv5 = tf.layers.conv2d(inputs=conv4,filters=4,kernel_size=[3, 1],padding="same",activation=tf.nn.relu,strides=[2,1])
    

    pool3 = tf.layers.max_pooling2d(inputs=conv5, pool_size=[1, 2], strides=[1,2])
    conv6 = tf.layers.conv2d(inputs=pool3,filters=136,kernel_size=[1, 3],padding="same",activation=tf.nn.relu,strides=[1,2])
